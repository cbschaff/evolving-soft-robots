...
import numpy as np
import gin
from training.wrappers import VecEpisodeLogger
import wandb
...
@gin.configurable(blacklist=['logdir'], module='coopt')
class SAC(Algorithm):
    """SAC algorithm."""
    ...
    def __init__(self,
        ...
        wandb.watch((self.pi, self.qf1, self.qf2), log="gradients", log_freq=10)
    ...
    def load(self, t=None):
        ...
        buffer_file = os.path.join(self.ckptr.ckptdir, 'buffer.npz')
        if os.path.exists(buffer_file):
            buffer_state = dict(np.load(buffer_file, allow_pickle=True))
